...
use hdk::holochain_core_types::{agent::AgentId, validation::EntryValidationData};
...
define_zome! {
    ...
    validate_agent: |validation_data : EntryValidationData::<AgentId>| {{
        if let EntryValidationData::Create{entry, ..} = validation_data {
            ...
            if agent.nick == "reject_agent::app" {
                Err("This agent will alfffways be rejected".into())
            } else {
                Ok(())
            }
        } else {
    ...
    functions: [
        ...
        join_channel: {
            inputs: | entry : channel::Channel |,
            outputs: |result: ZomeApiResult<Address>|,
            handler: channel::handle_join_channel
        }
        create_group: {
